...
2) Remap / calibrate offsets (force by default — pose robot at URDF home)
...
Examples:
  ...
  ros2 run quad_servo_driver hw_joint_test --ros-args -p force_recalibrate:=false -p delta_deg:=10.0
...
class HwJointTest(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("force_recalibrate", True)
        ...
        self.get_logger().warn(
            f"HW TEST: remap then +{delta_deg:.1f}° ({mode}). "
        ...
        try:
            ...
            self.get_logger().info(
                f"Remap done. Online joints ({len(online)}): {online}"
            )

            # Home all (sync write — start together)
            hw.enable_all_torque(True)
            home_targets = {
                name: hw.joint_config(name).urdf_home_rad for name in online
            }
            self.get_logger().info(
                f"Moving all online joints to URDF home simultaneously ({len(online)})…"
            )
            home_ok = hw.command_rad_many(home_targets)
            for name, ok in home_ok.items():
                self.get_logger().info(
                    f"  home {name} id={hw.joint_config(name).servo_id} ok={ok}"
                )
            ...
            if simultaneous:
                ...
                if return_home:
                    self.get_logger().info(
                        "Returning ALL to home simultaneously (sync write)…"
                    )
                    home_ok = hw.command_rad_many(home_targets)
                    time.sleep(settle)
                    for name in online:
